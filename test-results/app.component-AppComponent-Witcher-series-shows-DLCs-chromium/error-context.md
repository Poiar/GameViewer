...
- Name: app.component.spec.ts >> AppComponent >> Witcher series shows DLCs
- Location: e2e\app.component.spec.ts:65:7
...
Locator: locator('h3 a').filter({ hasText: 'The Witcher' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h3 a').filter({ hasText: 'The Witcher' }).first()
