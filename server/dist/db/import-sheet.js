...
function csvParseRow(line, rowNum) {
...
// ---------------------------------------------------------------------------
function inferEditionType(gameTitle, collectionTitle) {
    ...
    if (combined.includes("definitive edition") ||
...
async function main() {
    ...
    console.log(`[Import] Found: ${genreSet.size} genres, ${seriesSet.size} series, ${gameMap.size} unique games, ${collectionSet.size} collections`);
    ...
    const genreLinksAdded = new Set();
    for (const row of rows) {
        if (!row.game || !row.genre)
            continue;
        if (row.genre === "NA" || row.genre === "na")
            continue;
        const cleanGenre = row.genre.replace(/\?$/, "").trim();
        if (!cleanGenre || cleanGenre === "-")
            continue;
    ...
    const releaseIdMap = new Map(); // key: "gameSlug|provider|device" → release ID
    const collectionReleasePairs = [];
    ...
    for (const row of fullGameRows) {
        ...
        if (seenReleaseKeys.has(releaseKey))
            continue;
    ...
    const collectionTitleToId = new Map();
    // Group releases by collection
    const collReleases = new Map();
    ...
    console.log(`[Import] Created ${collectionTitleToId.size} collections with ${collectionReleasePairs.length} release links`);
    // -----------------------------------------------------------------------
    ...
    for (const row of dlcRows) {
        if (!row.collection || row.collection === "NA")
            continue;
    ...
    for (const row of fullGameRows) {
        if (!row.place || row.place === "NA" || row.place === "na")
            continue;
        if (!row.game)
            continue;
        const normalizedTitle = normalizeGameTitle(row.game);
    ...
    for (const seq of sequences) {
        try {
            await db.execute(sql `SELECT setval('${sql.raw(seq + "_id_seq")}', (SELECT COALESCE(MAX(id), 0) FROM ${sql.raw(seq)}))`);
        }
        catch {
