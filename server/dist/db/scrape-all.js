...
// --------------------------------------------------------------------------
async function scrapeOpenCritic(page, title) {
    try {
        return await page.evaluate(async ({ gameTitle, bearer }) => {
            ...
            if (!games.length)
                return null;
            const exact = games.find((g) => g.name.toLowerCase() === gameTitle.toLowerCase());
...
async function scrapeHltb(page, title) {
    try {
        ...
        const captured = new Promise((resolve) => {
            let settled = false;
            const done = (data) => { if (settled)
                return; settled = true; resolve(data); };
            const timer = setTimeout(() => done(null), 10_000);
            const handler = (resp) => {
                if (settled)
                    return;
                if (!resp.url().includes("/api/bleed"))
                    return;
                if (resp.request().method() !== "POST")
                    return;
                resp.json()
                    .then((j) => { if (j?.data?.length)
                    done(j); })
                    .catch(() => { });
            };
        ...
        if (!json?.data?.length)
            return null;
        const exact = json.data.find((g) => g.game_name?.toLowerCase() === title.toLowerCase());
        const match = exact ?? json.data[0];
        return {
            hltbId: match.game_id,
            hltbTime: match.comp_main ? Math.round(match.comp_main / 3600) : undefined,
        };
    }
    catch {
        return null;
...
// --------------------------------------------------------------------------
async function scrapeSteam(steamAppId) {
    try {
...
async function main() {
    ...
    const ocFails = new Map();
    const ocBlacklist = new Set();
    ...
    if (SCRAPE_OC)
        enrichConditions.push(isNull(masterGames.opencriticId));
    if (SCRAPE_HLTB)
        enrichConditions.push(isNull(masterGames.hltbId));
    ...
    const steamWhere = SCRAPE_STEAM
        ? and(sql `${masterGames.steamAppId} IS NOT NULL`, sql `(${masterGames.steamPlayers} IS NULL OR ${masterGames.steamPlayersAt} < NOW() - INTERVAL '1 hour')`)
        : null;
            ...
            if (games.length) {
                ...
                if (SCRAPE_OC && !g.opencriticId && ocPage) {
                    ...
                    if (oc?.opencriticId) {
                        ...
                        if (oc.criticScore != null)
                            sets.criticScore = oc.criticScore;
                        ocCount++;
                        ocFails.delete(g.id);
                ...
                if (SCRAPE_HLTB && !g.hltbId && hltbPage) {
                    ...
                    if (hltb?.hltbId) {
                        ...
                        if (hltb.hltbTime != null)
                            sets.hltbTime = hltb.hltbTime;
                        hltbCount++;
                ...
                if (!g.igdbId) {
                    ...
                    if (igdb) {
                        sets.igdbId = igdb.id;
                        if (igdb.summary && !g.summary)
                            sets.summary = igdb.summary;
                        if (!g.coverImageUrl) {
                            const cover = getIgdbCoverUrl(igdb.cover?.url);
                            if (cover)
                                sets.coverImageUrl = cover;
                        }
                        ...
                        if (ar) {
                            ...
                            const ratingMap = { 6: "RP", 7: "EC", 8: "E", 9: "E10+", 10: "T", 11: "M", 12: "AO" };
                        ...
                        // Franchise
                        if (igdb.franchise?.name)
                            sets.franchise = igdb.franchise.name;
                        // Steam AppID
                        const steamExt = igdb.external_games?.find((e) => e.external_game_source === 1);
                        if (steamExt)
                            sets.steamAppId = parseInt(steamExt.uid, 10) || undefined;
                        // Screenshots
                        if (!g.screenshots?.length && igdb.screenshots?.length) {
                ...
                if (Object.keys(sets).length > 1) {
                    await db.update(masterGames).set(sets).where(eq(masterGames.id, g.id));
                    didWork = true;
                }
            ...
            if (games.length) {
                ...
                if (st?.steamPlayers != null) {
                    await db
                        ...
                        steamPlayersAt: st.steamPlayersAt,
                        updatedAt: new Date(),
                    })
