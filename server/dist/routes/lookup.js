...
router.get("/platforms", async (_req, res) => {
    try {
        const rows = await db.select().from(platforms).orderBy(asc(platforms.name));
...
// GET /edition-types
router.get("/edition-types", async (_req, res) => {
    try {
        const rows = await db.select().from(editionTypes).orderBy(asc(editionTypes.name));
        res.json({ data: rows, error: null });
    }
    catch (error) {
...
// GET /media-formats
router.get("/media-formats", async (_req, res) => {
    try {
        const rows = await db.select().from(mediaFormats).orderBy(asc(mediaFormats.name));
        res.json({ data: rows, error: null });
    }
    catch (error) {
