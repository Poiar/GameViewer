...
import { collections as collectionsTable, collectionReleases, collectionDlcReleases, ownedInstances, } from "../db/schema.js";
...
router.get("/:id", optionalAuth, async (req, res) => {
    try {
        ...
        let ownedReleaseIds = new Set();
        let ownedDlcReleaseIds = new Set();
        if (req.user) {
            ...
            if (dlcReleaseIds.length > 0) {
                ...
                for (const o of ownedDlc) {
                    if (o.dlcReleaseId)
                        ownedDlcReleaseIds.add(o.dlcReleaseId);
                }
            }
        }
        // Shape the response
...
router.post("/", authenticate, validate(createCollectionSchema), async (req, res) => {
    try {
...
router.put("/:id", authenticate, validate(updateCollectionSchema), async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id)) {
            res.status(400).json({
        ...
        if (mediaFormatId !== undefined)
            updateData.mediaFormatId = mediaFormatId ?? null;
        if (releaseYear !== undefined)
            updateData.releaseYear = releaseYear ?? null;
...
router.post("/:id/releases/:releaseId", authenticate, async (req, res) => {
    try {
...
router.delete("/:id/releases/:releaseId", authenticate, async (req, res) => {
    try {
        const collectionId = parseInt(req.params.id, 10);
        const releaseId = parseInt(req.params.releaseId, 10);
        if (isNaN(collectionId) || isNaN(releaseId)) {
...
router.post("/:id/dlc-releases/:dlcReleaseId", authenticate, async (req, res) => {
    try {
        const collectionId = parseInt(req.params.id, 10);
...
router.delete("/:id/dlc-releases/:dlcReleaseId", authenticate, async (req, res) => {
    try {
        const collectionId = parseInt(req.params.id, 10);
        const dlcReleaseId = parseInt(req.params.dlcReleaseId, 10);
        if (isNaN(collectionId) || isNaN(dlcReleaseId)) {
        ...
        await db
            ...
            .where(and(eq(collectionDlcReleases.collectionId, collectionId), eq(collectionDlcReleases.dlcReleaseId, dlcReleaseId)));
        res.json({
