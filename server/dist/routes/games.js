...
import { masterGames, masterGameGenres, genres, series, releaseGroups, releases, providers, mediaFormats, ownedInstances, } from "../db/schema.js";
...
router.get("/", optionalAuth, async (req, res) => {
    try {
        ...
        const limit = Math.min(2500, Math.max(1, parseInt(req.query.limit) || 20));
        const offset = (page - 1) * limit;
        const search = req.query.search?.trim();
        const genreSlug = req.query.genre?.trim();
        const seriesId = parseInt(req.query.seriesId) || undefined;
        ...
        const conditions = [];
        if (search) {
            conditions.push(or(ilike(masterGames.title, `%${search}%`), sql `${masterGames.alternativeTitles}::text ILIKE ${`%${search}%`}`));
        }
        if (genreSlug) {
        ...
        if (req.user) {
            ...
            if (allDlcReleaseIds.length > 0) {
                const ownedDlc = await db
                    ...
                    .from(ownedInstances)
                    .where(and(eq(ownedInstances.userId, req.user.userId), inArray(ownedInstances.dlcReleaseId, allDlcReleaseIds)));
...
router.post("/", authenticate, validate(createGameSchema), async (req, res) => {
    try {
        ...
        if (genreIds && genreIds.length > 0) {
            await db.insert(masterGameGenres).values(genreIds.map((genreId) => ({
                gameId: created.id,
                genreId,
            })));
        }
        ...
        enrichGame(title).then((enrichment) => {
            if (enrichment.igdbId || enrichment.opencriticId || enrichment.hltbId) {
                ...
                db.update(masterGames).set(setData).where(eq(masterGames.id, created.id)).execute()
...
router.put("/:id", authenticate, validate(updateGameSchema), async (req, res) => {
    try {
        ...
        if (firstReleaseYear !== undefined)
            updateData.firstReleaseYear = firstReleaseYear;
        if (description !== undefined)
            updateData.description = description ?? null;
        if (coverImageUrl !== undefined)
            updateData.coverImageUrl = coverImageUrl ?? null;
        if (seriesId !== undefined)
            updateData.seriesId = seriesId ?? null;
        if (alternativeTitles !== undefined)
            updateData.alternativeTitles = alternativeTitles;
...
router.post("/:id/cover", authenticate, async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id)) {
...
router.post("/bulk-cover", authenticate, async (req, res) => {
    try {
