...
import { releases, releaseGroups, masterGames, providers, mediaFormats, editionTypes, dlcReleaseCompatibility, ownedInstances, } from "../db/schema.js";
...
router.get("/", optionalAuth, async (req, res) => {
    try {
        ...
        const gameId = parseInt(req.query.gameId) || undefined;
        const releaseGroupId = parseInt(req.query.releaseGroupId) || undefined;
        const platformSlug = req.query.platform?.trim();
        const providerSlug = req.query.provider?.trim();
        const search = req.query.search?.trim();
        const conditions = [];
        if (search) {
            conditions.push(or(ilike(masterGames.title, `%${search}%`), sql `${releases.playableOn}::text ILIKE ${`%${search}%`}`, sql `EXISTS (SELECT 1 FROM ${providers} p2 WHERE p2.id = ${releases.providerId} AND ${ilike(providers.name, `%${search}%`)})`));
        }
        if (releaseGroupId) {
        ...
        if (req.user && rows.length > 0) {
            ...
            for (const o of owned) {
                if (o.releaseId)
                    ownedReleaseIds.add(o.releaseId);
            }
        }
...
router.post("/", authenticate, validate(createReleaseSchema), async (req, res) => {
    try {
        const { releaseGroupId, title, providerId, mediaFormatId, intendedFor, playableOn, barcode, catalogNumber, publisher, region, releaseDate, controllerSupport, localMultiplayer, onlineMultiplayer, versionImageUrl, } = req.body;
...
router.put("/:id", authenticate, validate(updateReleaseSchema), async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id)) {
            res.status(400).json({
        ...
        if (releaseGroupId !== undefined)
            updateData.releaseGroupId = releaseGroupId;
        if (title !== undefined)
            updateData.title = title ?? null;
        if (providerId !== undefined)
            updateData.providerId = providerId;
        if (mediaFormatId !== undefined)
            updateData.mediaFormatId = mediaFormatId;
        if (intendedFor !== undefined)
            updateData.intendedFor = intendedFor;
        if (playableOn !== undefined)
            updateData.playableOn = playableOn;
        if (barcode !== undefined)
            updateData.barcode = barcode ?? null;
        if (catalogNumber !== undefined)
            updateData.catalogNumber = catalogNumber ?? null;
        if (publisher !== undefined)
            updateData.publisher = publisher ?? null;
        if (region !== undefined)
            updateData.region = region ?? null;
        if (releaseDate !== undefined)
            updateData.releaseDate = releaseDate ?? null;
        if (controllerSupport !== undefined)
            updateData.controllerSupport = controllerSupport;
        if (localMultiplayer !== undefined)
            updateData.localMultiplayer = localMultiplayer;
        if (onlineMultiplayer !== undefined)
            updateData.onlineMultiplayer = onlineMultiplayer;
        if (versionImageUrl !== undefined)
            updateData.versionImageUrl = versionImageUrl ?? null;
