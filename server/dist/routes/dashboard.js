...
import { masterGames, releases, series, collections, ownedInstances, userFavorites, masterGameGenres, genres, releaseGroups, collectionReleases, platforms, dlcReleases, dlcs, } from "../db/schema.js";
...
router.get("/stats", authenticate, async (req, res) => {
    try {
        ...
        // ── Global counts ──
        const [[{ count: totalGames }], [{ count: totalReleases }], [{ count: totalSeries }], [{ count: totalCollections }],] = await Promise.all([
        ...
        // ── Enrichment coverage ──
        const [[{ count: enrichedIgdb }], [{ count: enrichedOpenCritic }], [{ count: enrichedHltb }], [{ count: gamesWithCovers }],] = await Promise.all([
        ...
        ORDER BY count DESC
      `);
        const platformDistribution = platformRows.rows.map((r) => ({
            slug: r.slug,
            name: r.name,
        ...
        const [valueResult] = await db
            .select({
            total: sql `COALESCE(SUM(${ownedInstances.purchasePrice}::numeric), 0)`,
        })
            .from(ownedInstances)
            .where(eq(ownedInstances.userId, userId));
        ...
        for (const cc of collectionCompletenessData) {
            ...
            if (totalReleasesCount > 0) {
                const [{ count: ownedReleases }] = await db
                    ...
                    .innerJoin(ownedInstances, and(eq(ownedInstances.releaseId, collectionReleases.releaseId), eq(ownedInstances.userId, userId)))
