...
import { dlcs, dlcReleases, dlcReleaseCompatibility, masterGames, ownedInstances, } from "../db/schema.js";
import { authenticate, optionalAuth } from "../middleware/auth.js";
import { validate } from "../middleware/validate.js";
import { z } from "zod";
import { eq, and, desc, count, sql, inArray, ilike } from "drizzle-orm";
const router = Router();
// ---------------------------------------------------------------------------
// Validation schemas
// ---------------------------------------------------------------------------
        ...
        if (req.user && rows.length > 0) {
            ...
            if (dlcReleaseIds.length > 0) {
                ...
                for (const dr of dlcReleaseRows) {
                    if (ownedDrIds.has(dr.id))
                        ownedDlcIds.add(dr.dlcId);
                }
            }
        }
...
router.post("/", authenticate, validate(createDlcSchema), async (req, res) => {
    try {
...
router.put("/:id", authenticate, validate(updateDlcSchema), async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        if (isNaN(id)) {
            res.status(400).json({
        ...
        if (firstReleaseYear !== undefined)
            updateData.firstReleaseYear = firstReleaseYear ?? null;
        if (dlcType !== undefined)
            updateData.dlcType = dlcType;
...
router.post("/:dlcId/releases", authenticate, validate(createDlcReleaseSchema), async (req, res) => {
    try {
        const dlcId = parseInt(req.params.dlcId, 10);
        if (isNaN(dlcId)) {
            res.status(400).json({
                data: null,
...
router.post("/releases/:dlcReleaseId/compatibility", authenticate, async (req, res) => {
    try {
        const dlcReleaseId = parseInt(req.params.dlcReleaseId, 10);
...
router.delete("/releases/:dlcReleaseId/compatibility/:releaseId", authenticate, async (req, res) => {
    try {
        const dlcReleaseId = parseInt(req.params.dlcReleaseId, 10);
        ...
        await db
            ...
            .where(and(eq(dlcReleaseCompatibility.dlcReleaseId, dlcReleaseId), eq(dlcReleaseCompatibility.releaseId, releaseId)));
        res.json({
