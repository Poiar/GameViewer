...
import { ownedInstances, releases, dlcReleases } from "../db/schema.js";
...
router.get("/", async (req, res) => {
    try {
        ...
        const releaseIds = baseRows.map((r) => r.releaseId).filter(Boolean);
        const dlcReleaseIds = baseRows.map((r) => r.dlcReleaseId).filter(Boolean);
        // Batch query releases with full joins
        const releaseDataMap = new Map();
        const dlcReleaseDataMap = new Map();
...
router.post("/", validate(createInstanceSchema), async (req, res) => {
    try {
        const userId = req.user.userId;
...
router.put("/:id", validate(updateInstanceSchema), async (req, res) => {
    try {
        const userId = req.user.userId;
        ...
        if (releaseId !== undefined)
            updateData.releaseId = releaseId ?? null;
        if (dlcReleaseId !== undefined)
            updateData.dlcReleaseId = dlcReleaseId ?? null;
        ...
        if (acquiredDate !== undefined)
            updateData.acquiredDate = acquiredDate ?? null;
