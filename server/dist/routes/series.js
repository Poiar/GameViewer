...
import { z } from "zod";
import { eq, ilike, desc, asc, count, sql, and, inArray } from "drizzle-orm";
const router = Router();
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function isMainlineTitle(title) {
...
router.get("/", optionalAuth, async (req, res) => {
    try {
        ...
        if (sortBy === "name") {
            ...
            const gc = sql `(SELECT COUNT(*) FROM master_games WHERE master_games.series_id = series.id)`;
        ...
        const rows = await db
            ...
            gameCount: sql `(SELECT COUNT(*) FROM master_games WHERE master_games.series_id = series.id)`,
        })
        ...
        const coverMap = {};
        const mainlineMap = {};
        const seriesGamesMap = {}; // seriesId → gameIds
        if (seriesIds.length > 0) {
            ...
            for (const g of allGames) {
                if (!g.seriesId)
                    continue;
                if (!seriesGamesMap[g.seriesId])
                    seriesGamesMap[g.seriesId] = [];
                seriesGamesMap[g.seriesId].push(g.id);
                if (!mainlineMap[g.seriesId])
                    mainlineMap[g.seriesId] = 0;
                if (isMainlineTitle(g.title))
                    mainlineMap[g.seriesId]++;
                if (g.coverImageUrl && (!coverMap[g.seriesId] || coverMap[g.seriesId].length < 4)) {
                    if (!coverMap[g.seriesId])
                        coverMap[g.seriesId] = [];
        ...
        if (req.user && seriesIds.length > 0 && Object.keys(seriesGamesMap).length > 0) {
            ...
            const ownedPerSeries = await db
                ...
                gameCount: sql `COUNT(DISTINCT ${masterGames.id})::int`,
            })
                .from(ownedInstances)
...
router.post("/", authenticate, validate(createSeriesSchema), async (req, res) => {
    try {
        const { name, description } = req.body;
        const slug = slugify(name);
        const [existing] = await db.select({ id: series.id }).from(series).where(eq(series.slug, slug)).limit(1);
        if (existing) {
            res.status(409).json({ data: null, error: { code: "SLUG_TAKEN", message: `A series with the slug "${slug}" already exists` } });
            return;
        }
...
router.put("/:id", authenticate, validate(updateSeriesSchema), async (req, res) => {
    try {
        ...
        if (name !== undefined) {
            const slug = slugify(name);
            const [sc] = await db.select({ id: series.id }).from(series).where(and(eq(series.slug, slug), sql `${series.id} != ${id}`)).limit(1);
            if (sc) {
                res.status(409).json({ data: null, error: { code: "SLUG_TAKEN", message: `A series with the slug "${slug}" already exists` } });
                return;
            }
