...
// ---------------------------------------------------------------------------
// Pure-function extractions from error-handler.ts for isolated testing
// ---------------------------------------------------------------------------
...
function getClientMessage(err: AppError, isProduction: boolean): string {
...
describe("getErrorCode", () => {
  ...
  it("handles AppError interface without code property", () => {
    const err = new Error("Plain error") as AppError;
    expect(getErrorCode(err)).toBe("INTERNAL_SERVER_ERROR");
  });
});
