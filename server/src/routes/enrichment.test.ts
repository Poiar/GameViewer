...
// ---------------------------------------------------------------------------
// Pure-function extractions from enrichment.ts routes for isolated testing
// ---------------------------------------------------------------------------
 ...
 */
function buildEnrichmentSetData(enrichment: EnrichmentResult, existingCover: string | null): Record<string, unknown> {
 ...
 */
function shouldApplyIgdbCover(igdbCoverUrl: string | undefined, existingCover: string | null): boolean {
  if (!igdbCoverUrl) return false;
  if (igdbCoverUrl.includes("nocover")) return false;
  if (existingCover) return false;
  return true;
}
 ...
 */
function shouldLinkGenres(existingGenreCount: number, igdbGenreCount: number): boolean {
...
describe("buildEnrichmentSetData", () => {
  ...
  it("sets screenshots to undefined when empty array", () => {
    const enrichment: EnrichmentResult = { igdbScreenshots: [] };
    const data = buildEnrichmentSetData(enrichment, null);
  ...
  it("sets summary from igdbSummary", () => {
    const enrichment: EnrichmentResult = { igdbSummary: "A great game" };
    const data = buildEnrichmentSetData(enrichment, null);
    expect(data.summary).toBe("A great game");
  });

  it("sets criticScore from opencriticScore", () => {
    const enrichment: EnrichmentResult = { opencriticScore: 92 };
    const data = buildEnrichmentSetData(enrichment, null);
    expect(data.criticScore).toBe(92);
  });

  it("sets hltbTime", () => {
    const enrichment: EnrichmentResult = { hltbTime: 15 };
    const data = buildEnrichmentSetData(enrichment, null);
