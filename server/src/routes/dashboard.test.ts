...
interface RecentInstance {
  ...
  masterGameTitle: string;
  masterGameSlug: string;
  masterGameCover: string | null;
  playableOn: string[] | null;
...
describe("mergeRecentInstances", () => {
  const makeRelease = (overrides: Partial<RecentInstance> = {}): RecentInstance => ({
    id: 1,
  ...
  const makeDlc = (overrides: Partial<RecentInstance> = {}): RecentInstance => ({
    id: 100,
