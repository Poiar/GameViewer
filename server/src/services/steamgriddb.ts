...
export async function searchGame(title: string): Promise<SgdbGame | null> {
  ...
  const result = await sgdb<{ data: SgdbGame[] }>(`/search/autocomplete/${encoded}`);
...
export async function getBestGrid(gameId: number): Promise<SgdbGrid | null> {
...
export async function getBestHero(gameId: number): Promise<SgdbHero | null> {
  const result = await sgdb<{ data: SgdbHero[] }>(`/heroes/game/${gameId}?styles=alternate,blurred&limit=5`);
  if (!result?.data?.length) return null;
  return result.data.sort((a, b) => b.score - a.score)[0];
}

/** Fetch the best logo for a SteamGridDB game ID. */
export async function getBestLogo(gameId: number): Promise<SgdbLogo | null> {
  const result = await sgdb<{ data: SgdbLogo[] }>(`/logos/steam/${gameId}?limit=5`);
  if (!result?.data?.length) return null;
  return result.data.sort((a, b) => b.score - a.score)[0];
}
 ...
 */
export async function fetchCoverUrl(gameTitle: string): Promise<string | null> {
