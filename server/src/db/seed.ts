...
function systemSlug(system: string): string {
...
function unsureBoolString(val: string): string {
  const map: Record<string, string> = {
    yes: "Yes",
    no: "No",
    maybe: "Maybe",
...
function providerName(val: string): string {
  const map: Record<string, string> = {
    physical: "Physical",
    steam: "Steam",
    epic: "Epic",
...
// Provider name -> DB id
const providerLookup: Record<string, number> = {
  Physical: 1,
  Steam: 2,
  Epic: 3,
  GOG: 4,
...
const editionTypeLookup: Record<string, number> = {
  Original: 1,
  Remaster: 2,
  Remake: 3,
  Enhanced: 4,
...
const mediaFormatLookup: Record<string, number> = {
  "N/A": 1,
  Digital: 2,
  DVD: 3,
  CD: 4,
