...
export const masterGames = pgTable(
  "master_games",
  {
    ...
    // Is There Any Deal price tracking
    itadPlain: varchar("itad_plain", { length: 255 }),
    itadCurrentPrice: decimal("itad_current_price", { precision: 10, scale: 2 }),
    itadCurrentShop: varchar("itad_current_shop", { length: 100 }),
    itadCurrentUrl: varchar("itad_current_url", { length: 500 }),
    itadLowestPrice: decimal("itad_lowest_price", { precision: 10, scale: 2 }),
    itadLowestAt: timestamp("itad_lowest_at"),
    itadPricesAt: timestamp("itad_prices_at"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
  (table) => ({
