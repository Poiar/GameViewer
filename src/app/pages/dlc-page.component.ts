      ...
      <div class="search-wrap">
        ...
        @if (filter) {
          <button class="s-clear" (click)="filter = ''; onFilter()">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="s-x">
        ...
        @for (dlc of filtered(); track dlc.id; let i = $index) {
          <a class="dlc-row" [routerLink]="['/dlc', dlc.id]" [style.--idx]="i" [class.owned]="!!dlc.userOwns">
            <div class="dlc-left">
              ...
              @if (dlc.userOwns) {
                <span class="dlc-owned">✓ Owned</span>
              }
            </div>
...
export class DlcPageComponent implements OnInit {
  ...
  applyFilter(): void {
    ...
    this.filtered.set(
      this.dlcs().filter((d) => d.title.toLowerCase().includes(q) || d.gameTitle.toLowerCase().includes(q)),
    );
  }
