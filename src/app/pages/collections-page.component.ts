...
@Component({
  ...
  template: `
    <div class="c-header">
      ...
      <div class="ch-stat">
        <span class="chs-val">{{ total() }}</span
        ><span class="chs-lbl">Collections</span>
      </div>
    </div>
      ...
      <div class="skel-grid">
        @for (i of [0, 1, 2, 3, 4, 5]; track i) {
          <div class="skel-card">
            ...
            <div class="skel-l skel-l--t"></div>
            <div class="skel-l skel-l--m"></div>
          </div>
        }
      </div>
        ...
        @for (coll of filtered(); track coll.id; let i = $index) {
          <article class="c-card" [style.--idx]="i">
            <div class="cc-top">
              ...
              @if (coll.releaseYear) {
                <span class="cc-year">{{ coll.releaseYear }}</span>
              }
            </div>
            <h3 class="cc-title">{{ coll.title }}</h3>
            @if (coll.mediaFormat?.name) {
              <span class="cc-fmt">{{ coll.mediaFormat?.name }}</span>
            }
            <div class="cc-platforms">
              @for (p of platforms(coll); track p) {
                <span class="cc-chip">{{ p }}</span>
              }
            </div>
...
export class CollectionsPageComponent implements OnInit {
  ...
  applyFilter(): void {
    ...
    if (!q) {
      this.filtered.set(this.collections());
      return;
    }
