...
@Component({
  selector: "app-dlc-detail-page",
  standalone: true,
  imports: [RouterLink],
  template: `
    <a routerLink="/dlc" class="dd-back">← Back to DLCs</a>
    @if (loading()) {
      <div class="dd-load"><div class="spinner"></div></div>
    } @else if (error()) {
      <div class="dd-err">{{ error() }}</div>
    } @else if (dlc(); as d) {
      <div class="dd-hero">
        <h1>{{ d.title }}</h1>
        <div class="dd-meta">
          <span class="dd-type" [class]="'dt-' + d.dlcType.toLowerCase()">{{ d.dlcType }}</span>
          @if (d.firstReleaseYear) {
            <span class="dd-year">{{ d.firstReleaseYear }}</span>
          }
          @if (d.masterGame) {
            ...
            @for (dr of d.dlcReleases; track dr.id) {
              <div class="dd-release-row" [class.owned]="!!dr.userOwns">
                <div class="ddr-main">
                  ...
                  @if (dr.onDiscForConsoleOnly) {
                    <span class="ddr-disc">💿 On Disc</span>
                  }
                  @if (dr.releaseDate) {
                    <span class="ddr-date">{{ dr.releaseDate }}</span>
                  }
                </div>
                @if (dr.userOwns) {
                  <div class="ddr-owned">
                    <span class="ddr-badge">✓</span>
                    @if (dr.userOwns.condition) {
                      <span class="ddr-cond">{{ dr.userOwns.condition }}</span>
                    }
                    @if (dr.userOwns.location) {
                      <span class="ddr-loc" title="Location">📍 {{ dr.userOwns.location }}</span>
                    }
                    @if (dr.userOwns.purchasePrice) {
                      <span class="ddr-price" title="Price">💲 {{ dr.userOwns.purchasePrice }}</span>
                    }
                  </div>
                }
  ...
  styles: [
    `
      ...
      .dd-release-row:hover {
        background: var(--bg-elevated);
      }
      .dd-release-row + .dd-release-row {
        border-top: 1px solid var(--border-subtle);
      }
      .dd-release-row.owned {
...
export class DlcDetailPageComponent implements OnInit {
  ...
  dlc = signal<DlcDetail | null>(null);
  loading = signal(true);
  error = signal<string | null>(null);
  ngOnInit(): void {
    const id = this.route.snapshot.paramMap.get("id");
    if (!id) {
      this.error.set("No DLC specified");
      this.loading.set(false);
      return;
    }
    this.http.get<any>("/api/dlc/" + id).subscribe({
      next: (res) => {
        this.dlc.set(res.data ?? res);
        this.loading.set(false);
      },
      error: () => {
        this.error.set("DLC not found");
        this.loading.set(false);
      },
    });
  }
