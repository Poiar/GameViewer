...
@Component({
  ...
  template: `
    ...
    @if (loading()) {
      ...
      <div class="sd-error">
        <p>{{ error() }}</p>
        <a routerLink="/series">← All Series</a>
      </div>
    } @else if (series(); as s) {
      <div class="sd-hero">
        <h1>{{ s.name }}</h1>
            ...
            <a class="sd-card" [class.owned]="!!game.userOwns" [routerLink]="['/games', game.slug]" [style.--idx]="i">
              <div class="sdc-cover">
                @if (game.coverImageUrl) {
                  ...
                  <span class="sdc-ph">🎮</span>
                }
                @if (game.userOwns) {
                  <span class="sdc-owned">✓</span>
                }
              </div>
              <div class="sdc-body">
                <h3 class="sdc-title">{{ game.title }}</h3>
                <span class="sdc-year">{{ game.firstReleaseYear }}</span>
              </div>
            </a>
          }
        </div>
...
export class SeriesDetailPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!slug) {
      this.error.set("No series specified");
      this.loading.set(false);
      return;
    }
    this.http.get<any>("/api/series/" + slug).subscribe({
      next: (res) => {
        this.series.set(res.data ?? res);
        this.loading.set(false);
      },
      error: () => {
