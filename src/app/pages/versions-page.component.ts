    ...
    <div class="v-bar">
      <div class="v-search">
        ...
        <input
          class="vs-input"
          type="text"
          placeholder="Search by game title, platform, provider..."
          [(ngModel)]="searchQuery"
          (input)="onSearch()"
        />
        @if (searchQuery) {
    ...
    @if (loading()) {
      <div class="skel-list">
        @for (i of [0, 1, 2, 3, 4, 5, 6, 7]; track i) {
          <div class="skel-row">
            <div class="skel-l skel-l--g"></div>
            <div class="skel-l skel-l--p"></div>
            <div class="skel-l skel-l--f"></div>
          </div>
        }
      </div>
      ...
      <div class="empty">
        ...
        @if (searchQuery) {
          <button class="btn-r" (click)="searchQuery = ''; fetch()">Reset</button>
        }
      </div>
    } @else {
      <div class="v-list">
          ...
          <div class="v-row" [class.owned]="!!rel.userOwns" [style.--idx]="i">
            <div class="vr-game">
              @if (rel.masterGame?.title) {
                <a class="vr-title" [routerLink]="['/games', rel.masterGame?.slug ?? '']">{{
                  rel.masterGame?.title ?? rel.title
                }}</a>
              } @else {
