...
@Component({
  ...
  template: `
    <div class="tl-header">
      <h2>Timeline</h2>
      <p>{{ total() }} games across {{ decades().length }} decades</p>
    </div>
    @if (loading()) {
      <div class="tl-load"><div class="spinner"></div></div>
    } @else {
          ...
          <div class="tl-years">
            @for (year of yearsInDecade(decade[0]); track year) {
              @if (gamesByYear()[year]?.length) {
                <div class="tl-year">
                  <h4 class="tl-year-title">
                    {{ year }} <span class="tly-count">{{ gamesByYear()[year].length }} games</span>
                  </h4>
                  <div class="tl-grid">
                    @for (game of gamesByYear()[year]; track game.id; let i = $index) {
                      <a
                        class="tl-card"
                        [class.owned]="$any(game).ownedReleases?.length > 0"
                      ...
                      >
                        <div class="tlc-cover">
                          @if (game.coverImageUrl) {
                            ...
                            <span class="tlc-ph">🎮</span>
                          }
                          @if ($any(game).ownedReleases?.length > 0) {
                            <span class="tlc-owned">✓</span>
                          }
                        </div>
                        <div class="tlc-body">
                          <span class="tlc-title">{{ game.title }}</span>
                        </div>
                      </a>
                    }
                  </div>
...
export class TimelinePageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<any>("/api/games?limit=2000&sort=year&order=asc").subscribe({
      next: (res) => {
        const gs = res.data ?? res;
        this.games.set(gs);
        this.total.set(gs.length);
