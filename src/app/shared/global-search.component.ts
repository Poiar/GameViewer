            ...
            @if (searching()) {
              ...
              @for (r of results(); track r.url; let i = $index) {
                ...
                >
                  ...
                  @if (r.subtitle) {
                    <span class="gsr-sub">{{ r.subtitle }}</span>
                  }
                </a>
              }
            } @else {
...
export class GlobalSearchComponent {
  ...
  open(): void {
    ...
    setTimeout(() => (document.querySelector(".gs-input") as HTMLInputElement)?.focus(), 50);
  }
  close(): void {
        ...
        next: (res) => {
          ...
          (Array.isArray(games) ? games.slice(0, 4) : []).forEach((g: any) => {
            items.push({
              ...
              id: g.id,
              title: g.title,
              subtitle: String(g.firstReleaseYear ?? ""),
              slug: g.slug,
              url: g.slug ? `/games/${g.slug}` : `/games`,
            });
          });
          // Also search series
            ...
            next: (sRes) => {
              ...
              (Array.isArray(seriesList) ? seriesList.slice(0, 3) : []).forEach((s: any) => {
                items.push({
                  ...
                  id: s.id,
                  title: s.name,
                  subtitle: `${s.gameCount ?? s._count?.games ?? 0} games`,
                  url: s.slug ? `/series/${s.slug}` : `/series`,
                });
              });
              // Also search DLCs
  ...
  moveDown(e: Event): void {
    e.preventDefault();
    this.activeIdx.update((i) => Math.min(i + 1, this.results().length - 1));
  }
  moveUp(e: Event): void {
    e.preventDefault();
