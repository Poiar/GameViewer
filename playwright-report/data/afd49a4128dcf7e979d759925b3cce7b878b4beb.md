...
- Name: app.component.spec.ts >> AppComponent >> Warcraft series shows correct game
- Location: e2e\app.component.spec.ts:41:7
...
Locator: locator('h3 a').filter({ hasText: 'Warcraft' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h3 a').filter({ hasText: 'Warcraft' }).first()
