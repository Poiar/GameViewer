...
- Name: app.component.spec.ts >> AppComponent >> Metal Gear Solid series shows multiple games
- Location: e2e\app.component.spec.ts:48:7
...
Locator: locator('h3 a').filter({ hasText: 'Metal Gear Solid' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h3 a').filter({ hasText: 'Metal Gear Solid' }).first()
