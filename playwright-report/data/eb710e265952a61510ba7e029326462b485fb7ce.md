...
- Name: app.component.spec.ts >> AppComponent >> Red Dead series shows DLC under games
- Location: e2e\app.component.spec.ts:58:7
...
Locator: locator('h3 a').filter({ hasText: 'Red Dead' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('h3 a').filter({ hasText: 'Red Dead' }).first()
